...
export default {
  ...
  methods: {
    ...
    loadData(){
      axios.get(config.hostname+'expediente/obtener_expediente_area/2/'+this.page+'/'+this.perpage+'/'+this.id_user)
      .then( (response) =>{
        this.expedientes = response.data.data;
        this.allRows = Number(response.data.total);
        this.loadingTable = false;
    ...
    loadPerPage(perpage){
      if(this.showSearch == true){
        ...
        this.page = 1;
        axios.get(config.hostname+'expediente/obtener_expediente_area/2/1/'+perpage+'/'+this.id_user)
        .then( (response) =>{
          this.expedientes = response.data.data;
          this.allRows = Number(response.data.total);
        })
    ...
    loadPage(page){
      if(this.showSearch == true){
        ...
        .catch( (error) =>{
         ...
         axios.get(config.hostname+'expediente/obtener_expediente_area/2/'+page+'/'+this.perpage+'/'+this.id_user)
        .then( (response) =>{
          setTimeout(function(){document.body.scrollTop = 0; }, 50);
          this.expedientes = response.data.data;
          this.allRows = Number(response.data.total);
