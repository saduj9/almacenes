            ...
            <table class="table" v-if="expedientes.length != 0">
              <thead>
                <tr>
                  ...
                  <th class="hidden-sm-down" v-if="user.id_area_trabajo == 3 || 8">Mercurio</th>
              ...
              <tbody>
                <tr v-for="row in expedientes" :key="row.id_expediente" >
                  ...
                  <td v-if="user.id_area_trabajo == 3 || 8" class="text-semi-muted text-center">
                    <b-button variant="gray" size="sm" @click="openModalEdit(row.id_expediente,row.n_cantidad_hg)" v-if="row.n_cantidad_hg===null">
                      <b-icon icon="pencil-square" aria-hidden="true"></b-icon>
                    </b-button>
                    <b-modal :id="'modal-edit-'+row.id_expediente" title="Mercurio" header-bg-variant="dark" body-bg-variant="light" 
                     header-text-variant="white" :hide-footer="true">
                      <b-form>
                        <b-form-group id="input-group-hg" label="Ingrese cantidad:" label-for="input-hg">
                        <b-form-input
                          type="number"
                          id="input-hg"
                          v-model.number="hgInput"
                      ...
                      <div class="text-right">
                        ...
                        <b-button v-if="!loadingModal" class="ml-1" variant="info" @click="editHg(row.id_expediente,hgInput);">Editar</b-button>
                        <div class="data-loader">
                          <i class="la la-spinner la-spin" v-if="loadingModal"></i>
                        </div>
                      </div>
                    </b-modal>  
                    <p class="text-mercurio" @click="openModalEdit(row.id_expediente,row.n_cantidad_hg)" v-if="row.n_cantidad_hg!=null">
                      {{row.n_cantidad_hg}}</p>
                  </td>
                  ...
                  <td v-if="user.id_area_trabajo == 8 || user.id_area_trabajo == 3" class="text-center">
...
export default {
  ...
  methods: {
    ...
    editHg(id,hg){
      if(hg == null || hg == ''){
        this.$toasted.error('Ingrese un texto', {
          duration: 1500,
          position: 'top-center'
        });
      }else{
        this.loadingModal = true;
        axios.put(config.hostname+'expediente/actualizar_hg', 
        {
          "id_expediente": id,
          "n_cantidad_hg": hg
        },
    ...
    loadAll(){
      this.loadingTable = true;
      this.showTable = false;
      axios.get(config.hostname+'expediente/obtener_expediente_area/3/'+this.page+'/'+this.perpage+'/'+this.id_user)
      .then( (response) =>{
        if(response.data.status === 'success'){
          this.$toasted.success(response.data.message, {
            duration: 1500,
    ...
    loadPerPage(perpage){
      if(this.showSearch == true){
        ...
        this.page = 1;
        axios.get(config.hostname+'expediente/obtener_expediente_area/3/1/'+perpage+'/'+this.id_user)
        .then( (response) =>{
          this.expedientes = response.data.data;
          this.allRows = Number(response.data.total);
        })
    ...
    loadPage(page){
      if(this.showSearch == true){
        ...
        axios.get(config.hostname+'expediente/obtener_expediente_area/3/'+page+'/'+this.perpage+'/'+this.id_user)
        .then( (response) =>{
          setTimeout(function(){document.body.scrollTop = 0; }, 50);
          this.expedientes = response.data.data;
          this.allRows = Number(response.data.total);
